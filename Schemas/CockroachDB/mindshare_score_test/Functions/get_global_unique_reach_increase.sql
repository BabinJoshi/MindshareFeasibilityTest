-- mindshare_score_test.get_global_unique_reach_increase
-- CockroachDB port:
--   mindshare.  → mindshare_test.

CREATE OR REPLACE FUNCTION mindshare_score_test.get_global_unique_reach_increase(startdate bigint, enddate bigint)
...
WITH user_posts AS (
    ...
    LEFT JOIN mindshare_test.mindshare_user u ON u.x_id = user_x_id
...
unique_engagements AS (
    ...
    FROM user_posts up
    LEFT JOIN mindshare_test.user_post e
    ...
    LEFT JOIN mindshare_test.mindshare_user u ON u.x_id = e.user_x_id
),
...
post_sequence AS (
    ...
    GROUP BY root_user_id, root_post_id, root_tweet_created_at
),

sequenced_engagements AS (
    SELECT ue.*, ps.post_seq
...
audience_marked AS (
    SELECT
        se.*,
        CASE WHEN se.post_seq = fat.first_post_seq THEN TRUE ELSE FALSE END AS is_new_audience
...
post_metrics AS (
    SELECT
        ...
        SUM(CASE WHEN am.is_new_audience THEN am.engaged_user_score ELSE 0 END) AS expansion_unique_reach,
        COUNT(CASE WHEN am.is_new_audience THEN 1 END)                AS new_audience_count
    FROM audience_marked am
    GROUP BY am.root_post_id, am.root_user_id, am.root_username,
             am.root_tweet_created_at, am.post_seq
)

SELECT
    handle, username, root_post_id, root_tweet_created_at, post_seq,
    total_engagements, post_unique_reach, expansion_unique_reach, new_audience_count,
    SUM(new_audience_count) OVER (PARTITION BY handle ORDER BY post_seq) AS cumulative_new_audience_count,
    SUM(expansion_unique_reach) OVER (PARTITION BY handle ORDER BY post_seq) AS cumulative_expansion_unique_reach
FROM post_metrics
ORDER BY handle, root_tweet_created_at DESC
$q$
);
