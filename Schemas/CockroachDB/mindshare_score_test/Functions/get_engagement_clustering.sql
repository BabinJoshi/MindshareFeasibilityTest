-- mindshare_score_test.get_engagement_clustering
-- CockroachDB port:
--   'mindshare_score.mv_engagement_features_%s' → 'mindshare_score_test.mv_engagement_features_%s'

CREATE OR REPLACE FUNCTION mindshare_score_test.get_engagement_clustering(start_ts bigint, end_ts bigint, project_keyword text)
...
AS $function$
DECLARE
    view_name text := format('mindshare_score_test.mv_engagement_features_%s', lower(replace(project_keyword, ' ', '_')));
BEGIN
