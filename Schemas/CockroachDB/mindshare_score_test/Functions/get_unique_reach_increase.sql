-- mindshare_score_test.get_unique_reach_increase
-- CockroachDB port:
--   analytics.  → analytics_test.  (format string literal updated)
--   mindshare.  → mindshare_test.

CREATE OR REPLACE FUNCTION mindshare_score_test.get_unique_reach_increase(startdate bigint, enddate bigint, projectname text)
...
DECLARE
    ...
    sql_query  TEXT;
BEGIN
    table_name := 'analytics_test.mv_engagement_' || LOWER(projectname);
...
WITH user_posts AS (
    ...
    LEFT JOIN mindshare_test.mindshare_user mu
...
post_metrics AS (
    ...
    GROUP BY
        am.root_post_id, am.root_user_id, am.root_username,
