-- mindshare_score_test.create_user_posts_engagement_features_view
-- CockroachDB port:
--   analytics.  → analytics_test.
--   mindshare_score. → mindshare_score_test.
-- CRDB notes:
--   PERCENTILE_CONT WITHIN GROUP requires CRDB >= 23.1
--   RANGE BETWEEN interval FOLLOWING requires CRDB >= 22.2

CREATE OR REPLACE PROCEDURE mindshare_score_test.create_user_posts_engagement_features_view()
 LANGUAGE plpgsql
AS $procedure$
BEGIN
    DROP MATERIALIZED VIEW IF EXISTS mindshare_score_test.mv_user_posts_engagement_features;

    CREATE MATERIALIZED VIEW mindshare_score_test.mv_user_posts_engagement_features AS
    WITH base AS (
        SELECT
            root_post_id,
            root_user_id,
        ...
        FROM analytics_test.mv_user_posts_engagement
    ),
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS ix_mv_user_posts_engagement_features_root
        ON mindshare_score_test.mv_user_posts_engagement_features (root_post_id);
END;
$procedure$;
