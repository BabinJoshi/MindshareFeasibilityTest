-- mindshare_score_test.get_v2_analytics
-- CockroachDB port:
--   mindshare.  → mindshare_test.
--   mindshare_score. → mindshare_score_test.  (format %I references updated)
--   analytics.  → analytics_test.            (format %I references updated)
-- CRDB notes:
--   percentile_cont(0.5) WITHIN GROUP requires CRDB >= 23.1

CREATE OR REPLACE FUNCTION mindshare_score_test.get_v2_analytics(projectname text, startdate bigint, enddate bigint, sort_key text, private_user_ids text[] DEFAULT NULL::text[])
...
order_clause :=
    CASE sort_key
        WHEN 'MOST_POSTS'      THEN 'post_count + quote_post_count'
        WHEN 'MOST_VIEWS'      THEN 'post_view_count + quote_view_count'
        WHEN 'UNIQUE_ENGAGERS' THEN 'unique_engager'
        WHEN 'UNIQUE_REACH'    THEN 'unique_reach'
        WHEN 'REACH'           THEN 'reach'
        WHEN 'ENGAGEMENTS'     THEN 'engagements + replies'
        WHEN 'MINDSHARE_SCORE' THEN 'mindshare_score'
        WHEN 'SMART_REACH'     THEN 'smart_reach'
...
user_stats AS (
    SELECT
        user_x_id AS x_id,
        SUM(CASE WHEN is_post OR (is_quote AND NOT is_reply) THEN COALESCE(favorite_count, 0) + COALESCE(retweet_count, 0) ELSE 0 END) AS engagements,
...
user_content_scores AS (
    SELECT user_x_id AS x_id, AVG(content_score) AS avg_content_score
    FROM filtered WHERE (is_post OR (is_quote AND NOT is_reply))
    GROUP BY user_x_id
),

user_duration_p90 AS (
    SELECT f.user_x_id AS x_id, AVG(fe.duration_days_p90)::numeric AS average_p90
    FROM filtered f
    JOIN mindshare_score_test.%I fe ON fe.root_post_id = f.post_id
    WHERE f.is_post OR (f.is_quote AND NOT f.is_reply)
    GROUP BY f.user_x_id
),

community_score_total AS (
    SELECT COALESCE(SUM(COALESCE(mu.score, 0)), 0) AS community_score
    FROM (SELECT DISTINCT user_x_id FROM filtered) fp
    LEFT JOIN mindshare_test.mindshare_user mu ON mu.x_id = fp.user_x_id
),

with_scores AS (
    SELECT
        us.*, COALESCE(u.score, 0) AS x_score, COALESCE(u.x_username, '') AS x_username,
        ucs.avg_content_score, udp.average_p90
    FROM user_stats us
    LEFT JOIN mindshare_test.mindshare_user u ON u.x_id = us.x_id
    LEFT JOIN user_content_scores ucs ON ucs.x_id = us.x_id
    LEFT JOIN user_duration_p90   udp ON udp.x_id = us.x_id
),

engagement_stream AS (
    SELECT
        ws.x_id AS root_user_id, root_post_id,
        mve.engaged_user_id, mve.engaged_user_score, mve.is_engaged_reply
    FROM with_scores ws
    LEFT JOIN analytics_test.%I mve
...
unique_engagers AS (
    SELECT root_user_id, engaged_user_id, MAX(engaged_user_score) AS unique_engager_score
    FROM engagement_stream WHERE engaged_user_id IS NOT NULL
    GROUP BY root_user_id, engaged_user_id
),

post_level_unique_engager AS (
    SELECT DISTINCT ON (root_post_id, engaged_user_id)
        root_post_id, root_user_id, engaged_user_score
    FROM engagement_stream
),

reach_cte AS (
    SELECT root_user_id, SUM(engaged_user_score) AS reach
    FROM post_level_unique_engager GROUP BY root_user_id
),

engagement_totals AS (
    SELECT root_user_id, COUNT(*) FILTER (WHERE is_engaged_reply) AS replies_on_users_posts
    FROM engagement_stream GROUP BY root_user_id
),
...
unique_totals AS (
    SELECT
        ...
        percentile_cont(0.5) WITHIN GROUP (ORDER BY unique_engager_score)          AS median_unique_engagers_score,
        AVG(unique_engager_score)                                                   AS average_unique_engagers_score
    FROM unique_engagers GROUP BY root_user_id
),

valid_posts AS (
    SELECT root_post_id AS post_id, root_user_id AS handle
    FROM post_level_unique_engager GROUP BY root_post_id, root_user_id
),

unique_contributions AS (
    SELECT DISTINCT ON (cs.original_post_id, cs.replier_x_id)
        cs.original_post_id AS post_id, vp.handle, cs.contribution_score
    FROM mindshare_score_test.contribution_scores cs
    JOIN valid_posts vp ON vp.post_id = cs.original_post_id
...
post_scores AS (
    ...
    JOIN mindshare_test.mindshare_post mp
       ON mp.post_id = uc.post_id AND mp.project_keyword = $1
    GROUP BY uc.post_id, uc.handle, mp.content_score
),

user_post_scores AS (
    SELECT handle, SUM(post_smart_reach)::NUMERIC AS smart_reach, SUM(post_score)::NUMERIC AS user_post_score
    FROM post_scores GROUP BY handle
),

user_analytics AS (
    SELECT
        ws.x_id, ws.x_username, ws.x_score,
        ws.engagements + COALESCE(et.replies_on_users_posts, 0) AS engagements,
        ws.likes,
        COALESCE(et.replies_on_users_posts, 0) AS replies,
        ws.retweets,
        CASE WHEN COALESCE(et.replies_on_users_posts, 0) = 0 THEN 0
             ELSE ROUND(ws.likes::numeric / et.replies_on_users_posts, 2)
        END AS like_reply_ratio,
        ws.post_count, ws.quote_post_count, ws.replies_count,
        ws.post_view_count, ws.quote_view_count, ws.replies_view_count,
    ...
    FROM with_scores ws
    LEFT JOIN unique_totals    ut  ON ut.root_user_id  = ws.x_id
    LEFT JOIN engagement_totals et ON et.root_user_id  = ws.x_id
    LEFT JOIN reach_cte         rc ON rc.root_user_id  = ws.x_id
    LEFT JOIN user_post_scores  ups ON ups.handle      = ws.x_id
),

user_analytics_with_rank AS (
    SELECT *, ROW_NUMBER() OVER (ORDER BY mindshare_score DESC) AS user_rank
    FROM user_analytics
)
