-- mindshare_score_test.get_private_mindshare_leaderboard
-- CockroachDB port:
--   analytics.  → analytics_test.
--   mindshare.  → mindshare_test.
--   mindshare_score. → mindshare_score_test.

CREATE OR REPLACE FUNCTION mindshare_score_test.get_private_mindshare_leaderboard(startdate bigint, enddate bigint, projectname text, p_exclude_list text[] DEFAULT NULL::text[], p_private_user_list text[] DEFAULT NULL::text[])
                    ...
                    WHEN 'day' THEN
                        CASE
                            WHEN $8 IS NOT NULL THEN
                                $8 + (FLOOR(EXTRACT(EPOCH FROM (up.root_tweet_created_at - $8)) / 86400) * INTERVAL '1 day')
                            ELSE DATE_TRUNC('day', up.root_tweet_created_at)
                        END
                    WHEN 'week' THEN
                        CASE
                            WHEN $8 IS NOT NULL THEN
                                $8 + (FLOOR(EXTRACT(EPOCH FROM (up.root_tweet_created_at - $8)) / (7 * 86400)) * INTERVAL '7 days')
                            ELSE DATE_TRUNC('week', up.root_tweet_created_at + INTERVAL '1 day') - INTERVAL '1 day'
                        END
                    WHEN 'month' THEN
                        CASE
                            WHEN $8 IS NOT NULL THEN
                                $8 + (FLOOR(EXTRACT(EPOCH FROM (up.root_tweet_created_at - $8)) / (30 * 86400)) * INTERVAL '30 days')
...
filtered_scores AS (
    ...
    FROM scores
    WHERE
        ($4 IS NULL OR array_length($4, 1) IS NULL OR x_user_id = ANY($4))
        AND ($5 IS NULL OR array_length($5, 1) IS NULL OR x_user_id != ALL($5))
)
...
$q$, view_name);
    ...
    RETURN QUERY EXECUTE sql_query
        USING startdate, enddate, projectname,
              p_private_user_list, p_exclude_list,
              v_post_cap, v_cap_period, v_week_anchor;
