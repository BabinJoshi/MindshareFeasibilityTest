-- mindshare_score_test.calculate_all_decay_scores
-- CockroachDB port:
--   mindshare.  → mindshare_test.
--   mindshare_score. → mindshare_score_test.

CREATE OR REPLACE FUNCTION mindshare_score_test.calculate_all_decay_scores(p_reset_interval interval DEFAULT '30 days'::interval)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
    ...
    cnt     BIGINT;
BEGIN
    TRUNCATE mindshare_score_test.contribution_scores;
    ...
    LOOP
        ...
        PERFORM mindshare_score_test.calculate_decay_scores(proj.project_keyword, p_reset_interval);
