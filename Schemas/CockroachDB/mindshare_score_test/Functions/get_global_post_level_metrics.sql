-- mindshare_score_test.get_global_post_level_metrics
-- CockroachDB port:
--   mindshare.  → mindshare_test.
--   mindshare_score. → mindshare_score_test.

CREATE OR REPLACE FUNCTION mindshare_score_test.get_global_post_level_metrics(startdate bigint, enddate bigint)
...
WITH engagements AS (
    ...
    FROM mindshare_test.user_post reply_up
    JOIN mindshare_test.user_post root_up
        ON root_up.post_id = reply_up.root_post_id
    JOIN mindshare_test.mindshare_user root_u
        ON root_u.x_id = root_up.user_x_id
    JOIN mindshare_test.mindshare_user reply_u
...
ratio_data AS (
    ...
    JOIN mindshare_test.user_post p
