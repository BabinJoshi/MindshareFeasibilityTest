-- mindshare_score_test.create_engagement_clustering_features_view
-- CockroachDB port:
--   analytics.  → analytics_test.
--   mindshare_score. → mindshare_score_test.
-- CRDB notes:
--   PERCENTILE_CONT WITHIN GROUP requires CRDB >= 23.1
--   RANGE BETWEEN CURRENT ROW AND 3600 FOLLOWING (numeric) requires CRDB >= 22.2

CREATE OR REPLACE PROCEDURE mindshare_score_test.create_engagement_clustering_features_view(IN project_keyword text)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
    EXECUTE format('DROP MATERIALIZED VIEW IF EXISTS mindshare_score_test.%I CASCADE', features_view_name);

    EXECUTE format($sql$
        CREATE MATERIALIZED VIEW mindshare_score_test.%I AS
        WITH base AS (
            SELECT
                root_post_id,
                root_user_id,
    ...
    EXECUTE format('CREATE UNIQUE INDEX IF NOT EXISTS %I ON mindshare_score_test.%I (root_post_id)', index_name, features_view_name);
END;
$procedure$;
