-- mindshare_test.nucleus_post definition
-- CockroachDB port:
--   PARTITION BY LIST removed
--   CREATE INDEX ON ONLY → CREATE INDEX ON
--   schema renamed mindshare → mindshare_test

CREATE TABLE mindshare_test.nucleus_post (
...
CREATE INDEX ix_nucleus_post_post_created_at ON mindshare_test.nucleus_post (post_created_at);
CREATE INDEX ix_nucleus_post_post_id ON mindshare_test.nucleus_post (post_id);
CREATE INDEX ix_nucleus_post_user_x_id_time ON mindshare_test.nucleus_post (user_x_id, post_created_at);
