-- mindshare_test.mindshare_user definition
-- CockroachDB port: schema renamed mindshare → mindshare_test

CREATE TABLE mindshare_test.mindshare_user (
    ...
    last_score_fetched_at timestamptz DEFAULT (now() AT TIME ZONE 'utc'::text) NULL,
    CONSTRAINT mindshare_user_pkey PRIMARY KEY (x_id)
);

CREATE INDEX ix_mindshare_mindshare_user_x_username ON mindshare_test.mindshare_user (x_username);
