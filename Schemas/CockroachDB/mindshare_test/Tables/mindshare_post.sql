-- mindshare_test.mindshare_post definition
-- CockroachDB port:
--   PARTITION BY LIST removed (CRDB does not support Postgres declarative list partitioning)
--   CREATE INDEX ON ONLY → CREATE INDEX ON  (ON ONLY not supported in CRDB)
--   schema renamed mindshare → mindshare_test

CREATE TABLE mindshare_test.mindshare_post (
...
CREATE INDEX ix_mindshare_post_post_created_at ON mindshare_test.mindshare_post (post_created_at);
CREATE INDEX ix_mindshare_post_post_id ON mindshare_test.mindshare_post (post_id);
CREATE INDEX ix_mindshare_post_user_x_id_time ON mindshare_test.mindshare_post (user_x_id, post_created_at);
