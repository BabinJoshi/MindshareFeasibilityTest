...
--   The Postgres source has no PK, so calculate_decay_scores can INSERT duplicate rows
--   for the same (project_keyword, reply_post_id) without error (e.g. if run twice).
--   With this PK, a duplicate INSERT raises a unique-constraint violation and aborts.
--   When porting calculate_decay_scores, change the INSERT to use:
--     INSERT ... ON CONFLICT (project_keyword, reply_post_id) DO UPDATE SET ...
