-- mindshare.mindshare_user — CockroachDB port
--
-- Divergences from the Postgres source (Schemas/Mindshare/Tables/mindshare_user.sql):
--   * Indexes: dropped Postgres-only `USING btree`.
--   * No hotspot concern: PK is the high-cardinality x_id, not a sequential key.
--   * Like nucleus_user, plus a last_score_fetched_at column.
...
CREATE TABLE mindshare.mindshare_user (
    ...
    last_score_fetched_at TIMESTAMPTZ NULL     DEFAULT now(),
    CONSTRAINT mindshare_user_pkey PRIMARY KEY (x_id)
);

CREATE INDEX ix_mindshare_mindshare_user_x_username ON mindshare.mindshare_user (x_username);
